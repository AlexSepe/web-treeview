...
export const selectionFeatureSingle: FeatureImplementation = {
    key: "selection",
    ...
    itemInstance: {
        select: ({ tree, itemId }) => {
            ...
            tree.setSelectedItems(
                selectedItems.includes(itemId) ? selectedItems : [itemId]
        ...
        selectUpTo: ({ tree, item, itemId }, ctrl: boolean) => {
            ...
            const { selectedItems } = tree.getState();
            const uniqueSelectedItems = [...new Set([...selectedItems, ...newSelectedItems])];
